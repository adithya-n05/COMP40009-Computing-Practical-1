...
getAttribute str (Element _ list xmls)
  | res /= []
    = head res
  | otherwise
    = head ((dropWhile ( == "") (map (getAttribute str) xmls)) ++ [""])
  where
    res = [s2 | (s1, s2) <- list, str == s1]
getAttribute _ _
  = ""

getChildren :: String -> XML -> [XML]
getChildren str (Element name atts xmls)
  = filter (isGood str) xmls
getChildren _ _
  = []

isGood :: String -> XML -> Bool
isGood str (Element name _ _)
  = str == name
isGood _ _
  = False
...
getChild str xml
  ...
  | otherwise
    = head children
  where
    children = getChildren str xml

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild xml1 (Element name atts xmls)
  = (Element name atts (xmls ++ [xml1]))
...
getValue (Element _ _ xmls)
  = (Text (concatMap fromText (map getValue xmls)))
...
parseAttributes str
  ...
  where
    ...
    parseAttributes' s
      ...
      where
        (name, s1) = parseName s
        s2 = tail (skipSpace s1)
        s3 = tail (skipSpace s2)
        value = takeWhile (/='\"') s3
        s4 = dropWhile (/='\"') s3
        s5 = tail (skipSpace s4)
        (atts, str') = parseAttributes s5
parse :: String -> XML
...
parse' "" stack
  = (\(Element _ _ xmls) -> head xmls) (head stack)
parse' ('<' : '/' : str) stack
  = parse' str' (popAndAdd stack)
  where
    str' = tail (dropWhile (/= '>') str)
parse' ('<' : str) stack
  = parse' s2 ((Element name atts []) : stack)
  where
    (name, s1) = parseName str
    (atts, s2) = parseAttributes s1
parse' str stack
  = parse' str' (addChild (Text text) ((head stack)) : (tail stack))
  where
    text = takeWhile (/='<') str
    str' = dropWhile (/='<') str

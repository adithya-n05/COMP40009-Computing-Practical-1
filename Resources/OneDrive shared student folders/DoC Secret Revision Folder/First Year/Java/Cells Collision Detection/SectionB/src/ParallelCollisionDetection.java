...
public class ParallelCollisionDetection {
  ...
  private static void tests() throws Exception {
    test("tests/points100-no-collision.txt");
    test("tests/points100-with-collision.txt");
    test("tests/points-no-collision.txt");
    test("tests/points-with-collision.txt");
    test("tests/simple-no-collision.txt");
    test("tests/simple-with-collision.txt");
  }

  private static void test(String inputFile) throws Exception {
    PriorityQueueInterface<Object2D> sortedPoints = new PriorityQueue<>();
    AABB region = readAndSortObjects(inputFile, sortedPoints);
    boolean collisionFree = checkObjects(sortedPoints, region);
    if (collisionFree) {
   ...
   *
   * // collision detection: // We create a quadTree. // We try to add all the
   * 2D-objects to the quadTree. // At each step, we pick a 2D-object: // 1. we
   * get its safety region (i.e., a square centred at the current point and //
   * with width equals to twice of the point's size) // 2. by querying the
   * quadTree, we try to find if there is any existing 2D-object // within the
   * current point's safety region //   a. if there is none, then we add the
   * point to the quad tree //   b. otherwise, a collision is detected and we
   * halt and return false. // 3. if we successfully add all the points to the
   * quad-tree, then we are sure // that there is no collision between the
   * points, and we halt and return // true.
   */
  private static boolean checkObjects(
    ...
    AtomicBoolean noCollision = new AtomicBoolean(true);
    Thread one = new Thread(() -> {
      while (!sortedPoints.isEmpty()) {
        Object2D object;
        synchronized (sortedPoints) {
          object = sortedPoints.peek();
          sortedPoints.remove();
        }
        Point2D centre = object.getCenter();
        double size = 2 * object.getSize();
        double coordX = centre.x;
        double coordY = centre.y;
        AABB safetyRegion = new AABB(
            new Point2D(coordX - size, coordY - size),
            new Point2D(coordX + size, coordY + size)
        );
        synchronized (tree) {
          if (!tree.queryRegion(safetyRegion).isEmpty()) {
            noCollision.set(false);
            break;
          }
          tree.add(object);
        }

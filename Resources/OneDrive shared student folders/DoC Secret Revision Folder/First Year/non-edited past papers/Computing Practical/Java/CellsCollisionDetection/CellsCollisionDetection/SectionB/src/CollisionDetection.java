...
public class CollisionDetection {
  ...
  private static boolean checkObjects(
    ...
    QuadTree newTree = new QuadTree(region, 4);

    while (!sortedPoints.isEmpty()) {

      Object2D nextPoint = sortedPoints.peek();
      sortedPoints.remove();

      Point2D corner1 = new Point2D(
          2 * nextPoint.getSize() + nextPoint.getCenter().x,
          2 * nextPoint.getSize() + nextPoint.getCenter().y);

      Point2D corner2 = new Point2D(
          2 * nextPoint.getSize() - nextPoint.getCenter().x,
          2 * nextPoint.getSize() - nextPoint.getCenter().y);

      AABB nextRegion = new AABB(corner1, corner2);

      if (newTree.queryRegion(nextRegion).size() > 0) {
        return false;
      }

      newTree.add(nextPoint);
    }
    return true;
  }

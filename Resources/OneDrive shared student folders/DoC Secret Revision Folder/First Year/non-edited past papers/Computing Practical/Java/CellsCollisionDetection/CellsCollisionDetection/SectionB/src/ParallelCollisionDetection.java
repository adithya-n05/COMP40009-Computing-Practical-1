      ...
      Thread nextThread = new Thread( () -> {
          ...
          while (!sortedPoints.isEmpty()) {
            ...
            synchronized (sortedPoints) {
              nextPoint = sortedPoints.peek();
              sortedPoints.remove();
            }

            Point2D corner1 = new Point2D(
                nextPoint.getCenter().x + nextPoint.getSize() * 2,
                nextPoint.getCenter().y + nextPoint.getSize() * 2
            );
            ...
            synchronized (newTree) {
              if (newTree.queryRegion(new AABB(corner1, corner2)).size() > 0) {
                valid.set(false);
              }

...
public class Graph implements GraphInterface {
  ...
  private void setIncomingDegreeRecursively(GraphNode node) {
    ...
    node.getOutgoingEdges()
        .forEach(edge -> setIncomingDegreeRecursively(edge.getEndPoint()));
  }
  ...
  private void computeEarliestCompletionTime() {
    ...
    while (!sortedNodes.isEmpty()) {
      ...
      for (GraphEdge edge : node.getIncomingEdges()) {
        int time = edge.getStartPoint().getEarliestCompletionTime()
            + edge.getTaskDuration();
        if (max < time) {
          max = time;
        }
      }
      node.setEarliestCompletionTime(max);
    }
  }

  public int getEarliestCompletionTime() {
    computeEarliestCompletionTime();
    return finish.getEarliestCompletionTime();
  }
  ...
  private void parseGraph(String inputFile)
    ...
    Map<String, GraphEdge> tasks = new Hashtable<>();

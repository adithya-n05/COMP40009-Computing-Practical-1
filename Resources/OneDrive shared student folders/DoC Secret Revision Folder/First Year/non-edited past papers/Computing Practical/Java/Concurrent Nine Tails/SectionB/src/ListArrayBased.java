...
public class ListArrayBased<T> implements ListInterface<T> {
   ...
   * required
   * @throws ListIndexOutOfBoundsException if position is less than 1 or greater
  ...
  public void add(int givenPosition, T newItem)
    ...
    if (givenPosition < 1) {
      throw new ListIndexOutOfBoundsException("Position is less than 1");
    }
    if (givenPosition > length + 1) {
      throw new ListIndexOutOfBoundsException(
          "Position is greater than size+1 of the list"
      );
    }
  ...
  private void removeGap(int givenPosition) {
    // shift left items at position > givenPosition
    for (int pos = givenPosition + 1; pos <= length; pos++) {

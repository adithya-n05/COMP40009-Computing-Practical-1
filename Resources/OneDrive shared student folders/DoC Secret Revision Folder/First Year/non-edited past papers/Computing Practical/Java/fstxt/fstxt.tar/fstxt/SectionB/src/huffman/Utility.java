...
public class Utility {
  ...
  public static Map<String, Integer> countWords(List<String> words) {
    ...
    // return words.stream().collect(Collectors.toMap(w -> w, w -> 1, Integer::sum)); original


    /*return (Map<String, Integer>) IntStream
        .range(0, words.size() / 4)
        .mapToObj(x -> {
          Thread thisThread = new HuffmanThread(String.valueOf(x), words.subList(0, x * (words.size())));
          thisThread.start();
          return ((HuffmanThread) thisThread).getCountMap();
        }); */
  }

  private static class HuffmanThread extends Thread {

    private List<String> strings;
    private Map<String, Integer> countMap;

    public HuffmanThread(String valueOf, List<String> strings) {
      super(valueOf);

...
import Data.Maybe
import Data.List
import CmmProcPoint (procPointAnalysis)
...
update val@(id, num) xss@(x@(idx, numx):xs)
  | id == idx     = (id, num) : xs
  | otherwise     = x : update val xs
update item [] = [item]

apply :: Op -> Int -> Int -> Int
apply Add exp1 exp2 = exp1 + exp2
apply Mul exp1 exp2 = exp1 * exp2
apply Eq exp1 exp2  = isEqual exp1 exp2
apply Gtr exp1 exp2 = isGtr exp1 exp2
...
eval (Var id) xss@(x@(idx, valx):xs)
  | id == idx    = valx
  | otherwise    = eval (Var id) xs
eval (Apply op exp1 exp2) xss@(x:xs)
  = apply op (eval exp1 xss) (eval exp2 xss)


execStatement :: Statement -> State -> State
execStatement (Assign id exp) xss
  = update (id, (eval exp xss)) xss

execStatement (If exp block1 block2) xss
  | predicate == 1 = execBlock block1 xss
  | predicate == 0 = execBlock block2 xss
  where
    predicate = eval exp xss

execStatement (DoWhile block exp) xss = finalResult
  where
    doPart = execBlock block xss
    finalResult
      | checkLoop == 0 = doPart
      | checkLoop == 1 = execStatement (DoWhile block exp) doPart
      where
        checkLoop = eval exp doPart


execBlock :: Block -> State -> State
execBlock stms st = foldl (flip execStatement) st stms
...
foldConst (Phi num1 num2) 
  | num1 == num2 = num1 
  | otherwise    = Phi num1 num2

foldConst (Apply Add (Var x) (Const 0)) = Var x
foldConst (Apply Add (Const 0) (Var x) ) = Var x

foldConst (Apply op (Const num1) (Const num2)) 
...
sub :: Id -> Int -> Exp -> Exp
-- Pre: the expression is in SSA form
sub id num v@(Var idx) 
  | id == idx   = Const num
  | otherwise   = v 
sub id num (Apply op exp1 exp2)  = foldConst finalResult
  where
    part1 = sub id num exp1
    part2 = sub id num exp2
    finalResult = Apply op part1 part2
...
scan _ _ [] = ([], [])
scan id num (stm:stms) = finalResult 
  where
...
propagateConstants b = propagateConstants' b wlist
  where 
    (wlist, _) = scan "$INVALID" 0 b
    propagateConstants' :: Block -> Worklist -> Block
    propagateConstants' b wlist
